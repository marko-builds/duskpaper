...
def render(scene, out, width, height, seconds=120, fps=30, native=1280,
    ...
    cls, default_pal, _ = engines.get(scene)
...
def stop_live():
    ...
    _kill("mpvpaper")
    # restore Omarchy's current static wallpaper if that system is present
...
def main():
    ...
    for name in ("render", "set"):
        ...
        p.add_argument("--res", default="auto", help="WxH (default: detect)")
        ...
        p.add_argument("--palette", default=None, choices=engines.PALETTES)
        if name == "render":
    ...
    if a.cmd == "list":
        for name, (_, pal, desc) in engines.SCENES.items():
            print(f"  {name:10s} {desc}  [default palette: {pal}]")
        print(f"\npalettes: {', '.join(engines.PALETTES)}")

    elif a.cmd in ("render", "set"):
        if a.cmd == "set" and a.scene == "off":
            stop_live()
            return
        W, H = (detect_resolution() if a.res == "auto"
                else map(int, a.res.lower().split("x")))
        pal = a.palette or engines.get(a.scene)[1]
        default_out = (CACHE / f"{a.scene}_{pal}_s{a.seed}_{W}x{H}.mp4")
